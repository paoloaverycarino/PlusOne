...
const Calendar: React.FC = () => {
    ...
    const fetchDataForDay = async (dateString: string) => {
        const docRef = doc(db, "calendar", dateString);
                    ...
                    <div className="grid grid-cols-7 gap-4 w-full">
                        ...
                        {(() => {
                            ...
                            for (let i = 0; i < startDay; i++) {
                                blankSpaces.push(<div key={`blank-${i}`} className="h-20"></div>);
                            }
                            return blankSpaces;
                        })()}
                        ...
                        {daysInMonth.map((day) => (
                            <div
                                ...
                                onClick={() => fetchDataForDay(`${currentYear}-${(currentMonth + 1)
                                    .toString()
                        ...
                        <div className="display">
                            <div className="artboard artboard-demo phone-1 w-full h-full flex items-center justify-center">
                                {imageUrl ? (
                                    ...
                                    <p className="text-center text-lg font-semibold text-gray-500">Click a date to view the image.</p>
                                )}
                            </div>
                        </div>
                    </div>
